...
const { AuditoriaLei, DocumentosLei } = require("../../models/mutuario-lei.model");
...
router.post("/audicao-lei", async (req, res) => {
  ...
  try {
    ...
    if (trueValue.length === 7) {
      ...
      await log.create({
        // eslint-disable-next-line
        data: Date.now(), usuario: usuario_id, tabela: "L", operacao: `O documento ${descricao}, com ID: ${id_documento} foi auditado sem pendências.`,
      });
      console.log(documento);
    } else {
      await log.create({
        // eslint-disable-next-line
        data: Date.now(), usuario: usuario_id, tabela: "L", operacao: `O documento ${descricao}, com ID: ${id_documento} foi auditado com pendências para acerto.`,
      });
      const documento = await DocumentosLei.update(
        // Atualizar código do auditor na tabela documento após auditoria
        // eslint-disable-next-line
